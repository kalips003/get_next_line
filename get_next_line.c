...
int	rtrn_size(t_list *first_list, int fd, char *g_temp)
{
	t_list	*current_list;
	int		size;
	...
	while (is_there_n(current_list) <= 0)
	{
		...
		current_list = ini_node(current_list, g_temp);
		if (!current_list)
			return (-1);
...
t_list	*ini_node(t_list *first_list, char *g_temp)
{	
	t_list	*next_list;
	t_list	*current_list;
	int		i;
	...
	{
		i = 0;
		while (i++ < BUFFER_SIZE && g_temp[i - 1] != '\0')
...
////////////////////////////////////////
char	*f_rtrn_1(t_list *first_list, int fd, char *g_temp)
{
	int		size;
	char	*rtrn;
...
void	f_copy_rest(t_list *last_list, char *g_temp)
{
	int	i;
	int	size;
...
char	*get_next_line(int fd)
{
	...
	t_list		*current_list;
	char		*rtrn;
	static char	g_temp[BUFFER_SIZE];

	first_list = ini_node(NULL, g_temp);
	if (!first_list)
		return (NULL);
	...
	while (current_list->next != NULL)
		current_list = current_list->next;
	f_copy_rest(current_list, g_temp);
	free_all(&first_list);
	return (rtrn);
}
/*
